...
- **DECISÃO DELA (2026-09-04): resolver os links externos ao vivo ao gerar o arquivo do mês**,
  revertendo o `UpdateLinks=0` de 2026-08-11. Primeiro implementei com `UpdateLink`; **ela própria
  sugeriu abrir o arquivo de origem pra ele "ler sempre"** — testei a hipótese isolada (sem nenhum
  `UpdateLink`) e ela está certa: só de ter a base de contas aberta, a coluna T saiu de `#N/A` pra
  `4263000`. Trocado por `abrir_fontes_dos_links` / `fechar_fontes_dos_links` (abre a origem
  SOMENTE LEITURA, descobre o caminho pelo próprio `wb.LinkSources`, sem hardcode; `UpdateLink`
  virou plano B). **Ordem que importa: só fechar as fontes DEPOIS do Save** — fonte fechada antes,
  o Excel volta a gravar o valor em cache.
  **Testado com as funções de produção na ordem real:** `T=#N/A` → `4263000` / `Outras Despesas`,
  conferência PASSOU e o **Grand Total gravado em disco ficou 5.671.131,15**. Risco conferido: base
  de contas parada desde 16/07 e KSB1 de Julho salvo em 21/08, então isso mexe **só** nas 2 linhas
  da conta nova. Sincronizado na rede (18:05).
