...
---
## EM ANDAMENTO 2026-09-01 — Cockpit: Passo 3 (Provisões) e Passo 4 (Base Intermediária) fundidos numa aba só, botões numerados
...
- Decisão registrada em `memory/DECISOES.md` (entrada 2026-09-01, já a versão anterior "só
  texto" — **precisa adicionar uma segunda entrada** registrando que essa foi superada pela
  fusão de abas, ainda não feito).
...
parecia bem vazio). Cockpit tentado abrir 2x em background (job `b78ez9doy` falhou por
`psutil`, job `b3wl3xgs8` rodando após o install — **status final não confirmado ainda**,
alerta de sessão longa disparou no meio do trabalho).

**PENDENTE pra próxima ação/sessão:**
1. Confirmar que o job `b3wl3xgs8` (cockpit rodando em background) abriu a janela sem erro.
2. **Testar visualmente a aba ③ fundida** — abrir o cockpit, clicar na aba, conferir que os
   4 botões aparecem certos, que o clique de cada um ainda dispara a função certa (Atualizar
   Pivot KSB1 / Lançar Provisões / Atualizar Provisões / Finalização), e que o aviso de
   Janeiro (se for o mês) ainda aparece na aba certa (④ Rateio de Custos agora).
3. Atualizar `memory/reference_cockpit_fechamento_fitted.md` — ainda descreve "7 abas, uma
   por passo" (① Extração → ⑦ P&L); precisa virar 6 abas com a nova ③ fundida.
4. Re-sincronizar a cópia de rede do cockpit pro estagiário (`robocopy /MIR` pra
   `_Cockpit_KSB1\`, fica dessincronizada do GitHub por design — ver
   `reference_cockpit_fechamento_fitted.md`) — ainda NÃO feito, avisar a usuária antes.
