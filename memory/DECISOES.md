...
## 2026-09-04 — Links externos do BASE_KSB1 passam a ser resolvidos AO VIVO a cada geração (reverte a decisão de 2026-08-11)

**Decisão:** `gerar_ksb1_mensal.py` passa a **abrir o arquivo de origem do link (somente leitura) na mesma instância do Excel** enquanto monta e salva o arquivo do mês — novas funções `abrir_fontes_dos_links(excel, wb)` / `fechar_fontes_dos_links(abertos)`. Com a origem aberta, o Excel resolve a fórmula ao vivo contra ela, sem depender do cache do link. Antes, os links eram deliberadamente **não** atualizados (`UpdateLinks=0`, decisão de 2026-08-11).

**Por que abrir a fonte em vez de chamar `UpdateLink` (ideia da usuária, testada e confirmada):** a primeira implementação usava `wb.UpdateLink(...)`. Funciona, mas é uma chamada que pode **retornar sem erro e não ter atualizado nada** — foi exatamente o que pareceu acontecer no primeiro teste (na verdade a conta ainda não estava gravada, mas o episódio deixou claro que "UpdateLink disse OK" não prova nada). Abrir a origem é determinístico: testado isoladamente, **sem nenhuma chamada de `UpdateLink`**, a coluna T saiu de `#N/A` para `4263000` só por a base de contas estar aberta. `UpdateLink` ficou como plano B, para o caso de a fonte não poder ser aberta.

**Detalhe de ordem que importa:** as fontes só podem ser fechadas **depois** do `Save` do arquivo do mês — fechada a origem antes de salvar, o Excel volta a gravar o valor em cache. Por isso `fechar_fontes_dos_links` é chamada no `finally`, depois do Save e da conferência.

**Descoberta paralela (armadilha real):** a própria `Base_Contas_Contábeis_Fitted_22.xlsx` tem `<fileSharing readOnlyRecommended="1"/>`. A primeira tentativa de cadastrar a conta nela **falhou em silêncio** — o Excel abriu em modo leitura com `DisplayAlerts=False`, o `Save()` virou no-op e o script ainda imprimiu "salvo". `IgnoreReadOnlyRecommended=True` **não** resolveu (limitação já documentada em `remover_flag_somente_leitura_recomendada`). Solução usada num arquivo compartilhado: remover a flag do XML → gravar via COM → **restaurar a flag** no fim. Lição geral: depois de escrever xlsx via COM, **conferir no disco com openpyxl** — `wb.Save()` sem erro não prova que persistiu.
